...
if (!MONGODB_URI) {
  ...
  console.error('Please set MONGODB_URI in your Render.com environment variables to your MongoDB connection string.');
  process.exit(1);
}
...
async function appHandler(req, res) {
  try {
    ...
    if (page === 'dashboard') {
      ...
      const typeFilter = user.role === 'staff' ? 'staff' : 'student';
      data.notifs = await Notification.find({ type: typeFilter }).sort('-created_at');
